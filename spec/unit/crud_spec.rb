...
describe FlatDoc::Crud::ClassMethods do
  ...
  it "get a flatdoc" do
    ...
    it.should be_kind_of(FlatDoc::Mash)
  end
  
  it "return nil if it couldn't get a flatdoc" do
    @dbclass.should_receive(:get).with(1).and_return(nil)
    @class.get(1).should be_nil
  end
  
  it "return an error if it couldn't get! a flatdoc" do
    @dbclass.should_receive(:get).with(1).and_return(nil)
    lambda {@class.get!(1)}.should raise_error
  end
  
  it "create a flatdoc" do
    instance = FlatDoc::Mash.new('foo'=>'bar')
...
describe FlatDoc::Crud::InstanceMethods do
  
  before(:each) do
    setup_instance
  end
  
  it "instantiate a new flatdoc" do
    it = FlatDoc::Mash.new(:foo => 'bar').should == {'foo' => 'bar'}
  end
  
  it "saves and creates a flatdoc" do
    @db.should_receive(:create).and_return({'foo'=>'bar', 'id'=>1})
    it = FlatDoc::Mash.new(:foo => 'bar')
    it = @mash.save
    it.should be_kind_of(FlatDoc::Mash)
    it.should_not be_new_record
  end
  
  it "saves and updates flatdoc" do
